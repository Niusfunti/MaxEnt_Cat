...
df["name_month"] = df.apply(lambda row: f"Apus_apus_{row['month']}", axis=1)
...
df = df[["name_month", "x_UTM", "y_UTM", "N_indi"]]
df.to_csv(f"data/falsiot/falsiot_map.csv", index=False)
